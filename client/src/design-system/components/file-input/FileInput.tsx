...
export class FileInput extends React.PureComponent<Props, State> {
    ...
    renderConfirmation() {
        ...
        return (
            <button onClick={this.handleConfirmation}>{this.props.confirmBtnText} {this.state.file.name}</button>
        );
    }

    render() {
        return (
            <div className="file-input-wrapper" onDrop={this.handleDrag} onDragOver={this.handleDragOver}>
                <span>
                ...
                <input id="fileInput" accept="image/*" type="file" onChange={this.handleChange} />
            </div>
        );
    }
}
